// src/routes/auth.js — POST /auth/login | POST /auth/logout | GET /auth/me | PUT /auth/senha
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    // funcao_real: usa funcao_custom se definida, senão usa o perfil padrão
    const funcaoReal = user.funcao_custom || user.perfil;

    const token = jwt.sign(
      { sub: user.id, nome: user.nome, perfil: user.perfil,
        funcao: funcaoReal, is_admin: user.is_admin },
...
router.get('/me', authMiddleware, async (req, res) => {
  const { rows } = await query(
    `SELECT u.id, u.nome, u.email, u.cor_avatar, u.placa_veiculo,
            u.veiculo, u.telefone, u.ultimo_acesso, u.funcao_custom,
            p.nome AS perfil, p.is_admin, p.limite_adiantamento
  ...
  const u = rows[0];
  res.json({ ...u, funcao: u.funcao_custom || u.perfil });
});
