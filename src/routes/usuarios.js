// src/routes/usuarios.js — CRUD + controle de acesso + funcao_custom
...
router.post('/', adminOnly, async (req, res) => {
  const { nome, email, senha, perfil_id, funcao_custom,
          placa_veiculo, veiculo, telefone, cor_avatar } = req.body;
  ...
  try {
    ...
    const { rows: [usuario] } = await query(
      `INSERT INTO usuarios
         (nome, email, senha_hash, perfil_id, funcao_custom,
          placa_veiculo, veiculo, telefone, cor_avatar)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       RETURNING id, nome, email, status`,
      [nome.trim(), email.trim().toLowerCase(), senhaHash, perfil_id,
       funcao_custom || null,
       placa_veiculo || null, veiculo || null, telefone || null,
       cor_avatar || '#25D366']
    );
    await query(
    ...
    await auditLog({
      ...
      acao: 'LANCAMENTO',
      detalhe: 'Funcionário "' + nome + '" cadastrado' +
               (funcao_custom ? ' com função "' + funcao_custom + '"' : '') + '.',
...
router.put('/:id', adminOnly, async (req, res) => {
  const { nome, email, perfil_id, funcao_custom,
          placa_veiculo, veiculo, telefone, cor_avatar } = req.body;
  try {
    await query(
      `UPDATE usuarios
       SET nome = COALESCE($1, nome),
           ...
           funcao_custom = $4,
           placa_veiculo = COALESCE($5, placa_veiculo),
           veiculo = COALESCE($6, veiculo),
           telefone = COALESCE($7, telefone),
           cor_avatar = COALESCE($8, cor_avatar),
           updated_at = NOW()
       WHERE id = $9`,
      [nome, email ? email.toLowerCase() : null, perfil_id,
       funcao_custom || null,
       placa_veiculo, veiculo, telefone, cor_avatar, req.params.id]
    );
    await auditLog({
      usuarioId: req.user.id, usuarioNm: req.user.nome,
...
router.put('/:id/acesso', adminOnly, async (req, res) => {
  try {
    ...
    await auditLog({
      usuarioId: req.user.id, usuarioNm: req.user.nome,
      acao: 'ACESSO',
      detalhe: 'Acesso de "' + u.nome + '" alterado para ' + u.status + '.',
...
router.put('/:id/senha', adminOnly, async (req, res) => {
  ...
  try {
    ...
    await auditLog({
      usuarioId: req.user.id, usuarioNm: req.user.nome,
      acao: 'SENHA',
      detalhe: 'Senha de "' + u.nome + '" redefinida pelo administrador.',
      modulo: 'Controle de Acesso', ip: getIp(req),
    });
    res.json({ ok: true });
  } catch (err) {
...
router.put('/senha/lote', adminOnly, async (req, res) => {
  ...
  try {
    const hash = await bcrypt.hash(novaSenha, parseInt(process.env.BCRYPT_ROUNDS) || 10);
    let sql, params;
    if (usuario_ids && usuario_ids.length) {
    ...
    await auditLog({
      usuarioId: req.user.id, usuarioNm: req.user.nome,
      acao: 'SENHA',
      detalhe: 'Reset de senha em lote: ' + rowCount + ' usuário(s).',
      modulo: 'Controle de Acesso', ip: getIp(req),
    });
